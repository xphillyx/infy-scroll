/**
 * Infy Scroll
 * @file scripts.js
 * @author Roy Six
 * @license TBD
 */

/**
 * Scripts is an array of scripts that are custom-made for some popular websites to fix broken HTML or images due to the
 * unnatural way of how pages are appended. The custom scripts are only ran after each page is appended. These scripts
 * are usually intended to fix missing thumbnail images, but could serve to do anything to the appended elements.
 *
 * @type {*[]} the array of scripts
 */
var Scripts = [
  {
    // name:        "Google Search (Includes Videos and News Sections)",
    // description: "Thumbnail Image Script",
    // note:        "Uses complex inline scripts that are situationally added in the document body for each page to set the image sources. URLs are either hexadecimal encoded or unicode encoded and may point to Base 64 Images.",
    // author:      "Six",
    // todo: star ratings (background images)
    url: new Uint8Array([94, 104, 116, 116, 112, 115, 63, 58, 47, 47, 91, 94, 46, 47, 93, 43, 92, 46, 103, 111, 111, 103, 108, 101, 40, 63, 58, 92, 46, 91, 94, 46, 47, 93, 123, 50, 44, 51, 125, 41, 123, 49, 44, 50, 125, 47, 40, 63, 58, 99, 40, 63, 58, 115, 101, 124, 117, 115, 116, 111, 109, 41, 124, 115, 101, 97, 114, 99, 104, 124, 119, 101, 98, 104, 112, 124, 109, 124, 35, 41]),
    fn: function(doc) {
      [...doc.querySelectorAll("script")].forEach(script => {
        try {
          if (script && script.textContent) {
            const textContent = script.textContent;
            let regexp;
            // Type 1 Thumbnail Images
            // (function(){var s='data:image/jpeg;base64,';var ii=['vidthumb2','vidthumb3'];_setImagesSrc(ii,s);})();
            // URL has Hexadecimal Encoding. For example, '\x3d' is '='. Also, URLs point to Base 64 Images.
            // Example URL: data:image/jpeg;base64,\x3d
            // @see https://stackoverflow.com/a/4209150 by Gumbo
            // @see https://stackoverflow.com/a/4209128 by PleaseStand
            regexp = new RegExp("(var s=')(.*)(';)(var ii=\\[')(.*)('];)").exec(textContent);
            if (regexp && regexp[2] && regexp[5]) {
              // Note: var ii can have multiple vidthumbs so we need to split it, e.g. var ii=['vidthumb2','vidthumb3'];
              const ids = regexp[5].split("','");
              const url = regexp[2];
              // Decode Hexadecimal Encoding
              // Alternative Solution: JSON.parse('"' + url.replace(/([^\\]|^)\\x/g, '$1\\u00') + '"');
              const src = url.replace(/\\x([0-9A-Fa-f]{2})/gi, (...arguments) => { return String.fromCharCode(parseInt(arguments[1], 16))});
              for (const id of ids) {
                const img = doc.getElementById(id);
                if (src && img && img.nodeName && img.nodeName.toUpperCase() === "IMG") {
                  console.log("fn() - changing img id=" + id + ", from=" + img.src + " to=" + src);
                  img.src = src;
                }
              }
              return;
            }
            // Type 2 Thumbnail Images
            // (function(){google.ldidly=0;google.ldi={"vidthumb1":"url","vidthumb2":"url","vidthumb3":"url"}
            // URL has Unicode Encoding. For example, '\u003d' is '='
            // Example URL: https://encrypted-tbn1.gstatic.com/images?q\u003dtbn:ANd9GcQH8XUamt5Aejpl1Dkj9L5ihcN0uSgYOwQEmWQtCVsdfwpdDJ43bx-47OEP3Ko
            // @see https://stackoverflow.com/a/7885499 by Ioannis Karadimas
            // @see https://stackoverflow.com/a/43641559 by Thriggle
            regexp = new RegExp("(google.ldi={)(.*?)(})").exec(textContent);
            if (regexp && regexp[2]) {
              // Note: google.ldi={} can have multiple vidthumbs inside the braces so we need to split it by comma
              const objects = regexp[2].split(",");
              for (const object of objects) {
                const regexp2 = new RegExp("(\")(.*?)(\":\")(.*?)(\")").exec(object);
                if (regexp2 && regexp2[2] && regexp2[4]) {
                  const id = regexp2[2];
                  const url = regexp2[4];
                  const img = doc.getElementById(id);
                  // Decode Unicode Encoding
                  // Alternative Solution: JSON.parse("\"" + url + "\"");
                  const src = url.replace(/\\u([\d\w]{4})/gi, (...arguments) => { return String.fromCharCode(parseInt(arguments[1], 16))});
                  if (src && img && img.nodeName && img.nodeName.toUpperCase() === "IMG") {
                    console.log("fn() - changing img id=" + id + ", from=" + img.src + " to=" + src);
                    img.src = src;
                  }
                }
              }
            }
          }
        } catch(e) {
          console.log("fn() - error encountered, e=" + e);
        }
      });
    }
  },
  {
    // name:        "Bing Search",
    // description: "Thumbnail Image Script",
    // note:        "Uses a simple dataset url replacement on the image to set the image source and a complex replacement ala Google Search",
    // author:      "Six",
    url: new Uint8Array([94, 104, 116, 116, 112, 115, 63, 58, 47, 47, 40, 63, 58, 119, 119, 119, 124, 99, 110, 119, 101, 98, 41, 52, 63, 92, 46, 98, 105, 110, 103, 92, 46, 99, 111, 109, 47, 40, 63, 58, 91, 94, 47, 93, 43, 47, 41, 42, 63, 40, 63, 58, 114, 101, 115, 117, 108, 116, 115, 92, 46, 97, 115, 112, 120, 124, 115, 101, 97, 114, 99, 104, 41]),
    fn: function(doc) {
      // Type 1 Thumbnail Images (Simple)
      [...doc.querySelectorAll("img[data-src-hq][src='data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAEALAAAAAABAAEAAAIBTAA7']")].forEach(img => {
        img.src = img.dataset.srcHq ? img.dataset.srcHq : img.src;
      });
      // Type 2 Thumbnail Images (Complex - Same type as Google Search Type 1 Thumbnail Images)
      [...doc.querySelectorAll("script")].forEach(script => {
        if (script && script.textContent) {
          const textContent = script.textContent;
          let regexp = new RegExp("(var x=_ge\\(')(.*)('\\))(.*x.src=')(.*)(';)").exec(textContent);
          if (regexp && regexp[2] && regexp[5]) {
            const id = regexp[2];
            const url = regexp[5];
            const img = doc.getElementById(id);
            // This doesn't seem to need the hex decoding...
            // const src = url.replace(/\\x([0-9A-Fa-f]{2})/gi, (...arguments) => { return String.fromCharCode(parseInt(arguments[1], 16))});
            const src = url;
            if (src && img && img.nodeName && img.nodeName.toUpperCase() === "IMG") {
              console.log("fn() - changing img id=" + id + ", from=" + img.src + " to=" + src);
              img.src = src;
            }
            console.log("fn() - src=" + src);
          }
        }
      });
    }
  },
  {
    // name:        "Yahoo (News)",
    // description: "Thumbnail Image Script",
    // note:        "Uses a simple dataset url replacement on the image to set the image source. previews already work",
    // author:      "Six",
    url: new Uint8Array([94, 104, 116, 116, 112, 115, 63, 58, 47, 47, 40, 46, 43, 46, 41, 63, 40, 110, 101, 119, 115, 46, 41, 63, 40, 115, 101, 97, 114, 99, 104, 46, 124, 99, 104, 105, 101, 98, 117, 107, 117, 114, 111, 46, 41, 63, 121, 97, 104, 111, 111, 46, 40, 99, 111, 109, 124, 99, 111, 46, 106, 112, 41, 47, 40, 121, 104, 115, 47, 41, 63, 115, 101, 97, 114, 99, 104]),
    fn: function(doc) {
      [...doc.querySelectorAll("img:not([src])[data-src]")].forEach(img => {
        img.src = img.dataset.src ? img.dataset.src : img.src;
      });
    }
  },
  {
    // name:        "eBay /b (Category Search Results)",
    // description: "Thumbnail Image Script",
    // note:        "Uses a simple dataset url replacement on the image to set the image source.",
    // author:      "Six",
    // todo:        "Better url pattern"
    url: new Uint8Array( [104, 116, 116, 112, 115, 58, 47, 47, 119, 119, 119, 46, 101, 98, 97, 121, 46, 99, 111, 109, 47, 98, 47]),
    fn: function(doc) {
      [...doc.querySelectorAll("img[src='https://ir.ebaystatic.com/cr/v/c1/s_1x2.gif'][data-src]")].forEach(img => {
        img.src = img.dataset.src ? img.dataset.src : img.src;
      });
    }
  },
  {
    // name:        "NH",
    // description: "Thumbnail Image Script",
    // note:        "Uses a simple dataset url replacement on the image to set the image source.",
    // author:      "Six",
    url: new Uint8Array([94, 104, 116, 116, 112, 115, 63, 58, 47, 47, 110, 104, 101, 110, 116, 97, 105, 46, 110, 101, 116, 47]),
    fn: function(doc) {
      [...doc.querySelectorAll("img[data-src][src='data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7']")].forEach(img => {
        img.src = img.dataset.src ? img.dataset.src : img.src;
      });
    }
  },
  {
    // name:        "PH",
    // description: "Thumbnail Image Script",
    // note:        "Uses a simple dataset url replacement on the image to set the image source. previews already work",
    // author:      "Six",
    url: new Uint8Array([94, 104, 116, 116, 112, 115, 58, 47, 47, 91, 94, 46, 93, 43, 92, 46, 112, 111, 114, 110, 104, 117, 98, 92, 46, 99, 111, 109, 47]),
    fn: function(doc) {
      [...doc.querySelectorAll("img[data-thumb_url][src='data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7']")].forEach(img => {
        img.src = img.dataset.thumb_url ? img.dataset.thumb_url : img.src;
      });
    }
  },
  {
    // name:        "XN",
    // description: "Thumbnail Image Script",
    // note:        "Uses a simple dataset url replacement on the image to set the image source.",
    // author:      "Six",
    // todo:        "Make previews work",
    url: new Uint8Array([94, 104, 116, 116, 112, 115, 63, 58, 92, 47, 92, 47, 40, 63, 58, 91, 94, 46, 93, 43, 92, 46, 41, 42, 63, 120, 110, 120, 120, 92, 46, 99, 111, 109]),
    fn: function(doc) {
      [...doc.querySelectorAll("img[data-src][src*='/img/lightbox/lightbox-blank.gif']")].forEach(img => {
        img.src = img.dataset.src ? img.dataset.src.replace("THUMBNUM", "1") : img.src;
      });
    }
  },
  {
    // name:        "XV",
    // description: "Thumbnail Image Script",
    // note:        "Uses a simple dataset url replacement on the image to set the image source.",
    // author:      "Six",
    // todo:        "Make previews work",
    url: new Uint8Array([94, 104, 116, 116, 112, 115, 58, 47, 47, 119, 119, 119, 92, 46, 120, 118, 105, 100, 101, 111, 115, 92, 46, 99, 111, 109, 47]),
    fn: function(doc) {
      [...doc.querySelectorAll("img[data-src][src*='/img/lightbox/lightbox-blank.gif']")].forEach(img => {
        img.src = img.dataset.src ? img.dataset.src.replace("THUMBNUM", "1") : img.src;
      });
    }
  },
  {
    // name:        "YP",
    // description: "Thumbnail Image Script",
    // note:        "Uses a simple dataset url replacement on the image to set the image source.",
    // author:      "Six",
    url: new Uint8Array([94, 104, 116, 116, 112, 115, 58, 47, 47, 119, 119, 119, 46, 121, 111, 117, 112, 111, 114, 110, 46, 99, 111, 109]),
    fn: function(doc) {
      [...doc.querySelectorAll("img[data-thumbnail][src='data:image/gif,GIF89a%01%00%01%00%80%00%00%00%00%00%FF%FF%FF%21%F9%04%01%00%00%00%00%2C%00%00%00%00%01%00%01%00%00%02%01D%00%3B']")].forEach(img => {
        img.src = img.dataset.thumbnail ? img.dataset.thumbnail : img.src;
      });
    }
  }
];