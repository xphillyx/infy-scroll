 ...
 *
 * The following are the six main actions:
 * 1. next      - finds the next link
 * 2. prev      - finds the previous link
 * 3. increment - increments a URL
 * 4. decrement - decrements a URL
 * 5. button    - performs a button click (like a load more button)
 * 6. list      - uses a custom list of URLs (array) to append
 *
...
var Action = (() => {
   ...
   * @param document_ (optional) the current document to parse, used in nextPrev()
   * @public
   */
  async function performAction(action, caller, instance, items, callback, document_) {
    ...
    // Action: down, up, and off do not have actionPerformed ever set to true because they don't have badges
    switch (action) {
      case "down":
        down(instance, pages);
        break;
      case "up":
        ...
        break;
      case "next": case "prev":
        actionPerformed = nextPrev(action, caller, instance, document_);
        break;
    ...
    if (items.debugEnabled && actionPerformed && !(instance.autoEnabled || (caller === "auto" && instance.autoRepeat))) {
      ...
      chrome.runtime.sendMessage({greeting: "setBadge", badge: action, temporary: true}, function(response) { if (chrome.runtime.lastError) {} });
    }
    ...
    if (caller === "popupClickActionButton") {
      chrome.runtime.sendMessage({greeting: "updatePopupInstance", caller: caller, action: action, instance: instance}, function(response) { if (chrome.runtime.lastError) {} });
    }
  }
  ...
  function updateTab(caller, instance) {
    ...
    // Note: We don't want to call chrome.runtime.sendMessage to update the popup instance because it wakes up the background process every time an action is performed
   ...
   * @private
   */
  function nextPrev(action, caller, instance, document_) {
      ...
      fetch(instance.url, { method: "HEAD", credentials: "same-origin" }).then(function(response) {
        if (response && response.status &&
          ...
          if (!instance.autoEnabled) {
            chrome.runtime.sendMessage({greeting: "setBadge", badge: "skip", temporary: true, text: response.redirected ? "RED" : response.status + ""});
          }
        ...
        if (!instance.autoEnabled) {
          chrome.runtime.sendMessage({greeting: "setBadge", badge: "skip", temporary: true, text: "ERR"});
        }
        // Recursively call this method again to perform the action again and skip this URL, decrementing errorSkipRemaining
        incrementDecrementErrorSkip(action, caller, instance, errorSkipRemaining - 1);
      });
  ...
  function auto(instance) {
    console.log("auto() - instance.autoPaused=" + instance.autoPaused);
    let actionPerformed = false;
    if (instance.autoEnabled) {
  ...
  async function off(caller, instance) {
    ...
    await Promisify.setItems({"on": false});
    await Promisify.runtimeSendMessage({greeting: "turnOff"});
    ...
    await Promisify.runtimeSendMessage({greeting: "updatePopupInstance", caller: caller, action: "off", instance: instance});
  }
