...
var Saves = (() => {
  ...
  /**
   * Adds a new save. Saved URLs can either be Exact, Patterns, or Regular Expressions.
   *
  ...
  async function addSave(instance) {
    ...
    const save = {
      ...
      "scrollAction": instance.scrollAction, "scrollAppend": instance.scrollAppend, "scrollAppendScripts": instance.scrollAppendScripts, "scrollAppendStyles": instance.scrollAppendStyles,
      "scrollElementRule": instance.scrollElementRule, "scrollElementInsertRule": instance.scrollElementInsertRule, "scrollElementType": instance.scrollElementType, "scrollMediaType": instance.scrollMediaType,
      "nextType": instance.nextType, "nextSelector": instance.nextSelector, "nextXpath": instance.nextXpath, "nextAttribute": instance.nextAttribute, "nextKeywordsEnabled": instance.nextKeywordsEnabled,
      "prevType": instance.prevType, "prevSelector": instance.prevSelector, "prevXpath": instance.prevXpath, "prevAttribute": instance.prevAttribute, "prevKeywordsEnabled": instance.prevKeywordsEnabled,
  ...
  async function deleteSave(id, url, writeToStorage) {
    ...
    let saves = await Promisify.getItems("saves");
   ...
   *
   * There are four ways a URL can "match" an item in a list:
   * 1. Pattern - The default. Checks if a URL includes the characters in the pattern
   * 2. Regex - Regular Expression. Surrounded by / /
   * 3. Exact - Exact URL. Surrounded by * *
   * 4. Alternative Exact - For matching Database URLs. Surrounded by ( )
   *
  ...
  function matchesList(url, altURL, list, listName) {
    ...
    for (let item of list) {
      try {
        // Regex / /
        if (item.startsWith("/") && item.endsWith("/") && new RegExp(item.slice(1,-1)).test(url)) {
          result.type = "regex";
        }
        // Exact * *
        else if (item.startsWith("*") && item.endsWith("*") && item.slice(1,-1) === url) {
          result.type = "exact";
        }
        // Alt ( )
        else if (item.startsWith("(") && item.endsWith(")") && item.slice(1,-1) === altURL) {
          result.type = "alt";
        }
        // Pattern
        ...
        if (result.type) {
          console.log("matchesList() - Found a " + listName + " URL (" + result.type + ")");
          result.matches = true;
          result.url = item;
          break;
        }
  ...
  /**
   * Tests if a saved pattern matches a plaintext URL.
   *
   * @param url  the URL to match
