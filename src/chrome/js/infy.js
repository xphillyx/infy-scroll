...
var Infy = (() => {
  ...
  async function buildInstance(tab, items) {
          ...
          if (new RegExp(d.url).test(tab.url)) {
            ...
            if (nextURL && nextURL.url) {
              ...
              if (elements && elements.length > 0 && elements[0] && elements[0].parentNode) {
                console.log("buildInstance() - Database record found for this tab's url:" + d.url);
                let activate = false;
                ...
                object.databaseActivate = activate;
                break;
              }
            }
          }
    ...
    if (!object.hasOwnProperty("nextKeywordsEnabled")) {
      const findNextURLResponse = NextPrev.findNextPrevURL(object.nextType, object.nextSelector, object.nextXpath, object.nextAttribute, false, items.nextKeywords, object.decodeURIEnabled, false,  undefined);
      object.nextKeywordsEnabled = !findNextURLResponse || !findNextURLResponse.url;
    }
    if (!object.hasOwnProperty("prevKeywordsEnabled")) {
      const findPrevURLResponse = !NextPrev.findNextPrevURL(object.prevType, object.prevSelector, object.prevXpath, object.prevAttribute, false, items.prevKeywords, object.decodeURIEnabled, false,  undefined);
      object.prevKeywordsEnabled = !findPrevURLResponse || !findPrevURLResponse.url;
    }
    // Check to see if a custom script exists for this URL and set the instance's script attribute with the index in the Scripts array
    try {
      if (items.customScriptsEnabled) {
        for (let i = 0; i < Scripts.length; i++) {
          const scriptURL = new TextDecoder().decode(Scripts[i].url);
    ...
    const instance = {
      "accepted": false, "enabled": items.on && !!(object.saveFound || object.whitelistFound || (object.databaseFound && object.databaseActivate)),
      ...
      "nextType": object.nextType, "nextSelector": object.nextSelector, "nextXpath": object.nextXpath, "nextAttribute": object.nextAttribute, "nextKeywordsEnabled": object.nextKeywordsEnabled, "nextKeywords": items.nextKeywords,
      "prevType": object.prevType, "prevSelector": object.prevSelector, "prevXpath": object.prevXpath, "prevAttribute": object.prevAttribute, "prevKeywordsEnabled": object.prevKeywordsEnabled, "prevKeywords": items.prevKeywords,
      ...
      "autoTimes": items.autoTimes, "autoSeconds": items.autoSeconds, "autoSlideshow": items.autoSlideshow, "autoBehavior": items.autoBehavior, "autoBadge": items.autoBadge, "autoPaused": false, "autoRepeat": false, "autoRepeating": false, "autoRepeatCount": 0,
      "script": object.script,
      "scrollAction": object.scrollAction, "scrollAppend": object.scrollAction === "button" ? "none" : object.scrollAppend, "scrollElementRule": object.scrollElementRule, "scrollElementInsertRule": object.scrollElementInsertRule ? object.scrollElementInsertRule : "", "scrollElementType": object.scrollElementType, "scrollMediaType": object.scrollMediaType,
      ...
      "scrollAppendScripts": object.scrollAppendScripts, "scrollAppendStyles": object.scrollAppendStyles,
      "scrollbarExists": false, "scrollbarAppends": 0, "scrollDivider": items.scrollDivider, "scrollUpdateAddress": items.scrollUpdateAddress, "scrollUpdateTitle": items.scrollUpdateTitle, "scrollBehavior": items.scrollBehavior, "scrollOverlay": items.scrollOverlay,
      ...
      "currentPage": 1, "totalPages": 1, "isLoading": true, "started": false
    };
    // TODO: We only need the locationOrigin for the Popup's List mode to show the domain/origin. The origin has the protocol and hostname
    if (window && window.location) {
      instance.locationHostname = window.location.hostname;
      instance.locationOrigin = window.location.origin;
      instance.locationProtocol = window.location.protocol;
  ...
  async function getItems() {
    ...
    try {
      ...
      if (items && items.database && items.database.length > 0 && items.databaseAutoUpdate >= 1 && (!items.databaseDate || ((new Date() - new Date(items.databaseDate)) >= 86400000 * items.databaseAutoUpdate))) {
        ...
        const response = await Promisify.runtimeSendMessage({greeting: "downloadDatabase", options: {useBackup: false}, async: true});
